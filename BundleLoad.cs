...
public class BundleLoad : MonoBehaviour
{
    ...
    {
        // cache 폴더에 AssetBundle 을 담아야 하므로 캐싱 시스템이 준비될때까지 기다림
        while (!Caching.ready)
        {
            yield return null;
        }
        ...
        /*
            UnityWebRequestAssetBundle.GetAssetBundle(URL, version)
            를 사용하면 캐시를 우선 확인하고,
            같은 버전이 없으면 서버에서 다운로드한다.
        */
        ...
        using (UnityWebRequest request = UnityWebRequestAssetBundle.GetAssetBundle(bundleURL, version, 0))
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                 ...
                 * Cube 1, Cube 2, Cube 3 이 차례로 참조될 것이다.
                 */
                AssetBundleRequest assetRequest = bundle.LoadAssetAsync<GameObject>("Cube " + (i + 1));
                ...
                if (obj == null)
                {
                    Debug.LogWarning("Asset not found: Cube " + (i + 1));
                    continue;
                }
